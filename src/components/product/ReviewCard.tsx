...
export function CardRating({ product }: CardRatingProps) {
  ...
  return (
    <>
      ...
      <ReviewCard store={product.store} rating_filter={selectedFilter} productId={product.id} />
    </>
  )
}
...
export function ReviewCard({ productId, store, rating_filter }: ReviewCardProps) {
  const {
    data,
    fetchNextPage,
    hasNextPage,
