...
export default function CheckoutHeader({currentStep, index}:{currentStep:CheckoutStep, index:number}){
                        ...
                        <nav className="hidden md:flex items-center space-x-2 text-sm">
                            ...
                            {steps.slice(0, index + 1).map((s, index) => (
                                <React.Fragment key={index}>
                                    <i className="fas fa-chevron-right text-gray-400 text-xs"></i>
                                    <span className={s.key === currentStep ? "text-primary font-medium" : "text-gray-600"}>
                                        {s.label}
                                    </span>
                                </React.Fragment>
                            ))}
