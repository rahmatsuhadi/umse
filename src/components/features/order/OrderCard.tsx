    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-gray-800 text-sm sm:text-base">#{order.id}</h3>
            <p className="text-xs sm:text-sm text-gray-600">{formatDate(order.date)}</p>
          </div>
          <span className={`${order.statusClass} px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium`}>
            {order.statusText}
          </span>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-2 sm:p-3 mb-4">
          <div className="flex justify-between text-xs sm:text-sm">
            <span className="text-gray-600">
              {totalItems} item • {order.statusText}
            </span>
            <span className="font-semibold text-gray-800">Total: {formatPrice(order.total)}</span>
          </div>
        </div>
