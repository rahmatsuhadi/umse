...
export default function ProductStickyWA({ productId, productName, selectedVariantName, quantity, phone, isClosed }: ProductStickyWAProps) {
    ...
    return (
        <button
            onClick={handleClick}
            disabled={isClosed}
            className={`btn btn-lg ${isClosed ? '' : 'btn-whatsapp'}`}
            style={{
                textDecoration: "none", flexShrink: 0, border: "none",
                cursor: isClosed ? "not-allowed" : "pointer",
                opacity: isClosed ? 0.6 : 1,
                background: isClosed ? "#a0a0a0" : "#25D366",
                color: "white"
            }}
        >
            <svg viewBox="0 0 24 24" fill="currentColor" style={{ width: "20px", height: "20px", flexShrink: 0, opacity: isClosed ? 0.5 : 1 }}>
