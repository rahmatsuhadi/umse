...
export default function EditAddressPage({}: FormAddressPageProps) {
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const id = params.id;

  const { data, isLoading: isLoadingAddress } = useAddress(id!);
  const { mutate: handleUpdateAddress, isPending } = useUpdateAddress();
  const existingAddress = data?.data;

  const form = useForm<z.infer<typeof addressSchema>>({
    resolver: zodResolver(addressSchema),
    defaultValues: {
      address: "", district_id: "", is_primary: false,
      label: "", note: "", postal_code: "", province_id: "",
      regency_id: "", recipient_name: "", recipient_phone_number: "", village_id: "",
    },
  });
  ...
  useEffect(() => {
    if (existingAddress) {
      form.reset({
        ...
        province_id: existingAddress.province_id ? String(existingAddress.province_id) : "",
        regency_id: existingAddress.regency_id ? String(existingAddress.regency_id) : "",
        district_id: existingAddress.district_id ? String(existingAddress.district_id) : "",
        village_id: existingAddress.village_id ? String(existingAddress.village_id) : "",
  ...
  // watch fields for fetching dependent location queries
  ...
  return (
    <div style={{ background: "var(--cream, #FFF9F4)", minHeight: "100vh" }}>
      {/* Sub-header */}
      <SubHeader title="Edit Alamat" backHref="/pengguna/alamat" />
        ...
        {isLoadingAddress ? (
          ...
          >
            <div style={{
              ...
              color: "#E74C3C", margin: "0 auto 16px",
            }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-muted)", margin: "0 0 24px" }}>
              Alamat yang ingin Anda edit tidak tersedia.
            </p>
            <button
              onClick={() => router.back()}
              style={{
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "var(--text-muted)", margin: 0 }}>Perbarui data penerima & lokasi</p>
              </div>
            </div>
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        <Textarea
                          ...
                          style={{
                            ...
                            border: "1.5px solid var(--cream-dark, #F0D5C2)",
                            paddingLeft: 40, paddingTop: 10,
                            fontSize: 14, minHeight: 64, resize: "vertical",
                          }}
                          {...field}
                        />
                      </div>
  ...
  return (
    <FormField
      ...
      render={({ field }: { field: { value: string } }) => {
        ...
        const selectedItem = (items as Array<{ id: number | string; name: string }>)
          .find((item) => String(item.id) === String(field.value));
